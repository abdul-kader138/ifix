...
class Sales extends MY_Controller
{
    ...
    public function repair_list($warehouse_id = null)
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('Repair')));
        $meta = array('page_title' => lang('Repair'), 'bc' => $bc);
        $this->page_construct('sales/repair_list', $meta, $this->data);
    }


    public function getRepair($warehouse_id = null)
    {
        $this->sma->checkPermissions('index');
        ...
        $repair_link = '<a class="repair_status_history" id="$1" href="#"><i class="fa fa-history"></i>Repair History</a>';
//        $repair_link = anchor('admin/sales/repair_history/$1', '<i class="fa fa-history"></i> ' . lang('Repair_History'));
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, ".$this->db->dbprefix('companies') .".name, phone, (grand_total+COALESCE(rounding, 0)), paid, (grand_total-paid) as balance,type_detail,sale_status, payment_status, companies.email as cemail")
                ->from('sales')
            ...
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, ".$this->db->dbprefix('companies') .".name, phone, (grand_total+COALESCE(rounding, 0)), paid, (grand_total+rounding-paid) as balance, type_detail,sale_status, payment_status, companies.email as cemail")
                ->from('sales')
                ->join('companies', 'companies.id=sales.customer_id', 'left')
                ->group_by('sales.id');
        }
        $this->datatables->where('pos', 1);
        $this->datatables->where('repair_status', 1);
        if (!$this->Customer && !$this->Supplier && !$this->Owner && !$this->Admin && !$this->session->userdata('view_right')) {
            $this->datatables->where('created_by', $this->session->userdata('user_id'));
        } elseif ($this->Customer) {
            $this->datatables->where('customer_id', $this->session->userdata('user_id'));
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products)) {
            ...
            if($inv->repair_status =='1')admin_redirect("sales/repair_list");
            else admin_redirect($inv->pos ? "pos/sales" : "sales");
        } else {
    ...
    public function topup_gift_card($card_id)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->topupGiftCard($data, $card_data,$ca_data)) {
    ...
    public function add_gift_card()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('card_no' => $this->input->post('card_no'),
                ...
                'customer' => $customer,
                'award_point'=>($ca_points ? $ca_points : 0),
    ...
    public function repair_status($id)
    {
        ...
        $inv = $this->sales_model->getStatusByID($id);
        if ($this->form_validation->run() == true) {
            $status = $this->input->post('status');
            $note = $this->sma->clear_tags($this->input->post('repair_note'));
            $repair_status = array(
                'sale_id' => $inv->id,
...
//        if ($this->form_validation->run() == true) {
        if ($this->form_validation->run() == true && $this->sales_model->updateRepairStatus($inv->id, $repair_status, $sale_status)) {
            $this->session->set_flashdata('message', lang('status_updated'));
            admin_redirect(isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : 'sales/repair_list');
        } else {
            $this->data['inv'] = $inv;
            $this->data['user'] = $this->sales_model->getUsers($inv->tech_id);
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'sales/repair_status', $this->data);
        }
    }
    ...
    {
//        $this->sma->checkPermissions(false, true);
        $this->data['payments'] = $this->sales_model->getRepairDetails($id);
        $this->data['modal_js'] = $this->site->modal_js();
        $this->load->view($this->theme . 'sales/repair_history', $this->data);
    }
